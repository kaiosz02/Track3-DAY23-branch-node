    ...
    *** MUST use a real LLM call — keyword-only heuristics will lose points. ***
    ...
    try:
        ...
        if any(kw in query_lower for kw in RISKY_KEYWORDS):
            ...
            w in query_lower for w in ("timeout", "failure", "cannot recover", "error", "crash")
        ):
            route, risk_level = "error", "low"
        elif any(
            w in query_lower for w in ("lookup", "order", "status", "tracking", "search", "12345")
        ):
            route, risk_level = "tool", "low"
        elif (
            any(w in query_lower for w in ("fix it", "help", "can you", "what"))
            and len(query_lower) < 20
        ):
